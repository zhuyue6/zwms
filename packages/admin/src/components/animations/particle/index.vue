<template>
  <canvas ref="canvasRef" class="particle-container"></canvas>
</template>
...
var RENDERER = {
  ...
  setParameters: function (strategy) {
    this.$container = canvasRef.value
  ...
  changeAngle: function (event) {
    ...
    this.rotationX =
      ((this.center.y - y) / this.center.y) * this.MAX_ROTATION_ANGLE
    this.rotationY =
      ((this.center.x - x) / this.center.x) * this.MAX_ROTATION_ANGLE
  },
  ...
  drawFigure: function () {
    ...
    for (let y = 0; y < this.particles.length; y++) {
      this.particles[y].rotateX(this.rotationX)
      this.particles[y].rotateY(this.rotationY)
    }
...
var STRATEGY = {
  ...
  createVase: function () {
    var theta = Math.random() * Math.PI,
      x =
        Math.abs((this.SCATTER_RADIUS * Math.cos(theta)) / 2) +
        this.SCATTER_RADIUS / 8,
