<template>
  <div ref="domRef" class="bg-container"></div>
</template>
...
let [camera, scene, renderer, sw, sh] = []
var mouse = { x: 0, y: 0 }
...
function init() {
  ...
  function listen(eventNames, callback) {
    for (let i = 0; i < eventNames.length; i++) {
      domRef.value.addEventListener(eventNames[i], callback)
    }
  }
  ...
  if (box !== emptySlot) {
    ...
    if (!box.isWarping && Math.random() > 0.999) {
      ...
      if (boxes[yn][zn] && boxes[yn][zn][xn] === emptySlot) {
        ...
        if (dir === 4) {
          // slide vertically
          gsap.to(box.position, 0.5, {
            y: yi * planeOffset,
          })
        } else {
          // slide horizontally
          gsap.to(box.offset, 0.5, {
            x: box.offset.x + xo * (size.width + gap),
            z: box.offset.z + zo * (size.depth + gap),
          })
        }
        gsap.to(box.offset, 0.6, {
          onComplete: function () {
            box.isWarping = false
          },
        })
...
useListenWindowOnresize(function () {
  sw = domRef.value.clientWidth
  sh = domRef.value.clientHeight
  camera.aspect = sw / sh
